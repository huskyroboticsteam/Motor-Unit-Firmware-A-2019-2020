...
{
    CyRamVectors[CYINT_IRQ_BASE + isr_manual__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + isr_manual__INTC_NUMBER];
}
...
void isr_manual_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((isr_manual__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *isr_manual_INTC_PRIOR = (*isr_manual_INTC_PRIOR & (uint32)(~isr_manual__INTC_PRIOR_MASK)) |
...
uint8 isr_manual_GetPriority(void)
{
    ...
    priority = (*isr_manual_INTC_PRIOR & isr_manual__INTC_PRIOR_MASK) >> priorityOffset;
